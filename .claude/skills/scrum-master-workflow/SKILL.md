...
> ⚠️ **スタック型ブランチ（複数IssueをA→B→Cとスタックして実装した場合）は `git diff main...最上位ブランチ` に全Issueの変更が含まれる（Sprint 54で発生）。**
> 最上位ブランチを対象にすると全変更がまとまってしまいreviewerがどのファイルがどのIssueの変更か判別できない。
> SMが `git show [コミットハッシュ] --name-only --format=""` でコミット単位の変更ファイルを取得し、reviewer に各Issue固有のファイルリストを渡すこと。

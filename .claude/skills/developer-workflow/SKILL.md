...
### 再発防止ルールのライフサイクル（2回ルール・卒業）

チェックリスト系のルール（レビュー指摘・バグの再発防止項目）は以下のライフサイクルで管理する。**追加だけを続けるとチェックリストが単調増加し、本当に効かせたい項目が埋もれるため、昇格と卒業をセットで運用する。**

```
1回目の指摘 → long_term.md「繰り返し指摘されるパターン」に記録（待機）
2回目の指摘 → Skillのチェックリストへ昇格（発生スプリントを併記）
長期間未発生＋理由を説明できる → 卒業（long_term.md「卒業済みルール」へ降格）
卒業後に再発 → 即座にチェックリストへ復帰（2回ルールは適用しない）
```

**昇格基準（2回ルール）:**

- レビュー指摘・バグの再発防止ルールは、**初出時は long_term.md に記録するだけで Skill のチェックリストには追加しない**
- Retroで long_term.md の「発生スプリント」欄を確認し、同種の指摘が**2スプリント以上**で発生していたら Skill のチェックリスト・規約へ昇格させる（発生スプリント・Issue番号を併記）
- **例外（1回目から即Skillに追加してよいもの）**: 手順・実装パターン・API仕様・ID類など「知らないと書けない参照知識」。2回ルールの対象は「注意すれば防げる系」のチェックリスト項目のみ

**卒業基準（Retroの棚卸しで判定）:**

- 対象: 本Skillの「コミット前チェック」・各conventionsスキルのチェックリスト・long_term.md「繰り返し指摘されるパターン」
- **必要条件**: 直近15スプリントで同種の指摘・再発がない（long_term.md の「発生スプリント」欄で確認する）
- **十分条件**: 未発生の**理由**を以下のいずれかで説明できること。説明できない場合は卒業させず次回Retroまで保留する
  - モデルが一般知識で対応できる内容になった
  - コード構造・設計の変化でそもそも起きなくなった
  - lint・Hooks・コード生成などで機械的に防げるようになった
- 卒業したルールは削除せず、long_term.md「卒業済みルール」へ移す（卒業スプリント・理由を併記）
- 卒業は1回のRetroにつき**最大3件**まで（一度に消しすぎて穴を開けない）
- 卒業を実施したら `#skills-changelog` に `[DEV]` プレフィックスで「何を・なぜ卒業したか」を投稿する
...
3. **Skillsファイルの更新要否を判断する：**
   ...
   - **再発防止ルールの追加は「再発防止ルールのライフサイクル」の2回ルールに従う**（初出のレビュー指摘は long_term.md 記録に留め、Skillには追加しない）
   - 更新すべき内容があれば更新する
...
4. **チェックリストの棚卸し（卒業判定）を実施する：**
   - 「再発防止ルールのライフサイクル」の卒業基準に従い、直近15スプリントで未発生かつ理由を説明できるルールを卒業させる（1回のRetroで最大3件）
   - 卒業したルールはSkill・チェックリストから削除し、`memory/dev/long_term.md` の「卒業済みルール」へ移す（卒業スプリント・理由を併記）
   - 卒業を実施したら `#skills-changelog` に `[DEV]` プレフィックスで投稿する
   - 該当がなければ「卒業候補なし」としてよい

5. **`memory/dev/short_term.md` をリセットする：**
   ```

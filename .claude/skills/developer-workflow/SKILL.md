...
1. **計画フェーズ完了時（実装方針承認・TaskCreate後）**: `#20-sprint` の作業スレッドに実装方針を投稿する
...
4. **承認を得たら TaskCreate で実装タスクを作成する**
   - **TaskCreateの前に必ず `TaskList` を確認し、既存タスクがないことを確認する**。既存タスクがあれば新規作成せず既存を活用する
...
5. **`#20-sprint` の作業スレッドに承認済み実装方針を投稿する：**
...
6. **実装方針を `memory/dev/short_term.md` に記録する**
7. **SMにSendMessageで報告してここで作業を止める：**
   ```
...
2. **`TaskList` で既存タスクを確認する**
   - pending タスクがあれば → **新規 TaskCreate しない**。既存タスクを `TaskUpdate(status: in_progress)` で更新して使用する
   - タスクが1件もなければ → 計画フェーズのTaskCreateが失敗している。実装方針に基づき自分でTaskCreateしてから進める

...
- **`_runCatching()` のシグネチャは Notifier の種類ごとに個別定義した**: 共通基底クラスは不要とのりょこさん確認のため、各 Notifier に private メソッドとして追加した。Generic で共通化できるが、Dart の型システム上 `state.copyWith(errorMessage: msg)` のような呼び出しを Generic にするには `copyWith` の型を抽象化する必要があり過設計になるため見送り。

...
ダークモード有効時に、OnboardingCardが白背景・黒文字のまま表示され、ライトモードと区別がつかない状態になっている。Sprint 19 Sprint Reviewにてりょこさんが確認（スクリーンショットあり）。
...
- Sprint 19 Sprint Review にてりょこさんが指摘（スクリーンショットあり）

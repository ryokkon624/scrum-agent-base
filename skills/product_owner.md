...
**バージョン**: 1.5.0
**最終更新**: 2026-04-22
**更新理由**: bug用IssueのBodyフォーマット追加（発生事象・原因・改修方針セクション）
...
1. `ryokkon624/hw-hub-manage` のIssue一覧を取得する
   - **ターミナル（CLI）起動時**：`mcp__github__list_issues` を使用（Private repoへのアクセス可能、2026-04-22確認）
   - **デスクトップアプリ起動時**：curlを使う（MCPではPrivate repoにアクセス不可）
   ```bash
   curl -s \

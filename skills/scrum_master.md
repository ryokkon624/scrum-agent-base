...
**バージョン**: 1.8.0
**最終更新**: 2026-04-20
**更新理由**: GitHub Projects連携・product_backlog.md廃止・Planning手順更新
...
1. **GitHub MCPで対象Issueを取得する：**
   - `ryokkon624/hw-hub-manage` リポジトリのIssueを取得する
   - Sprint列に今回のSprintの番号が記入されているIssueを対象とする
   - `Ready: Ready` のIssueを優先する
   - 各IssueのBodyからACを確認する
2. スプリントゴールを策定する
3. リスクとチャレンジ項目を明示する
...
2. **`#40-retrospective` に新スレッドを作成**してRetroを実施する：
   ...
   - 回避すること（Avoid）
   - チャレンジすること（Challenge）
3. りょこさんの指摘を **GitHub MCPで `ryokkon624/hw-hub-manage` にIssueを作成する**（Ready: Draft）
